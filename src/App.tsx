...
const Sidebar = ({ currentView, setCurrentView, handleLogout, currentUser }) => (
  <div className="bg-emerald-800 text-white w-64 flex-shrink-0 hidden md:flex flex-col min-h-screen print:hidden">
    ...
    <div className="p-6 border-t border-emerald-700">
      <div className="text-xs text-emerald-300 mb-4 px-2 flex items-center">
        <User className="w-4 h-4 mr-2" /> {currentUser?.username} ({currentUser?.role})
      </div>
      <button 
      ...
      <p className="text-xs text-emerald-400 text-center mt-4">Sistem Keuangan Masjid v3.2</p>
    </div>
  </div>
);
...
const MobileMenu = ({ isMobileMenuOpen, setIsMobileMenuOpen, setCurrentView, handleLogout, currentUser }) => {
...
const LoginView = ({ onLogin, accounts }) => {
...
const DashboardView = ({ balance, totalIncome, totalExpense, setCurrentView, transactions, currentUser }) => (
...
const TransactionForm = ({ type, incomeCategories, expenseCategories, onSave, onCancel }) => {
...
const SettingsView = ({ incomeCategories, setIncomeCategories, expenseCategories, setExpenseCategories }) => {
  ...
  const removeIncomeCat = (catToRemove) => {
    setIncomeCategories(incomeCategories.filter(cat => cat !== catToRemove));
  };

  const removeExpenseCat = (catToRemove) => {
    setExpenseCategories(expenseCategories.filter(cat => cat !== catToRemove));
  };
    ...
    <div className="space-y-6 max-w-4xl">
      ...
      {!isFirebaseReady && (
        <div className="bg-yellow-50 border border-yellow-200 text-yellow-800 p-4 rounded-lg text-sm">
          <strong>Perhatian:</strong> Anda sedang dalam Mode Demo. Penambahan kategori ini hanya tersimpan di memori dan akan hilang jika halaman dimuat ulang.
        </div>
      )}
...
const AccountSettingsView = ({ currentUser, accounts, setCurrentUser }) => {
  ...
  const handleChangePassword = async (e) => {
    ...
    if (isFirebaseReady) {
      try {
        ...
        setPasswordMsg({ type: 'error', text: 'Gagal mengubah password di database!' });
      }
    }
  };
  ...
  const handleAddAccount = async (e) => {
    ...
    if (isFirebaseReady) {
      try {
        ...
        setAccountMsg({ type: 'error', text: 'Gagal menambahkan akun ke database!' });
      }
    }
  };
        ...
        {currentUser?.role === 'admin' && (
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div className="bg-blue-600 p-4 text-white">
              <h3 className="font-bold flex items-center">
                <User className="w-5 h-5 mr-2" /> Tambah Akun Baru
              </h3>
            </div>
...
export default function App() {
  ...
  // Fitur fallback jika database error
    ...
    const initAuth = async () => {
      try {
        ...
        if (error.code === 'auth/admin-restricted-operation') {
          setDbError("Satu Langkah Terakhir! Anda belum mengaktifkan fitur 'Anonymous' di pengaturan Firebase Authentication.");
        } else {
  ...
  // --- FIREBASE FETCH DATA ---
  useEffect(() => {
    // Jika fallback aktif, muat data dummmy dan hentikan loading
    if (isFallbackMode) {
    ...
    if (!isFirebaseReady || !authUser) return;

    const timeoutId = setTimeout(() => {
      if(isLoadingData) {
        setDbError("Koneksi lambat/gagal. Pastikan pengaturan Rules Firestore sudah benar dan jaringan internet Anda stabil.");
        setIsLoadingData(false);
      }
    }, 10000);

    // Listener untuk Firestore: Akun Pengguna
    const unsubAccounts = onSnapshot(collection(db, 'accounts'), (snapshot) => {
      if (snapshot.empty) {
        // Jika belum ada akun, buat akun default. Menggunakan catch agar tidak error jika akses ditolak
        addDoc(collection(db, 'accounts'), { username: 'admin', password: 'admin123', role: 'admin' }).catch(e => console.log('Pending write'));
        addDoc(collection(db, 'accounts'), { username: 'user', password: 'user123', role: 'user' }).catch(e => console.log('Pending write'));
      } else {
      ...
      console.error("Error akun:", error);
      if (error.code === 'permission-denied' || error.message.includes('Missing or insufficient permissions')) {
        setDbError("permission-denied");
      } else {
        setDbError("Gagal mengambil data akun. " + error.message);
      }
      clearTimeout(timeoutId);
      setIsLoadingData(false);
    });

    // Listener untuk Firestore: Transaksi
    const unsubTx = onSnapshot(collection(db, 'transactions'), (snapshot) => {
      ...
      if (error.code === 'permission-denied' || error.message.includes('Missing or insufficient permissions')) {
        ...
        setDbError("Gagal mengambil data transaksi. " + error.message);
      }
      clearTimeout(timeoutId);
      setIsLoadingData(false);
    });
  ...
  const handleAddTransaction = async (newTx) => {
    if (isFallbackMode) {
       // Simpan di memori lokal saat fallback mode
    ...
    if (isFirebaseReady) {
      try {
        ...
        alert("Gagal menyimpan ke database! Pastikan Rules Firestore Anda mengizinkan 'write'.");
      }
    }
  };
  ...
  // TAMPILAN ERROR KHUSUS KONEKSI DATABASE
  if (dbError) {
           ...
           <div className="bg-white p-8 rounded-xl max-w-2xl border border-red-200 shadow-xl w-full">
             ...
             <div className="text-left text-gray-700 space-y-4 mb-8">
               ...
               <div className="bg-blue-50 p-5 rounded-lg border border-blue-200">
                 <h3 className="font-bold text-blue-800 mb-2">Cara Mengatasinya (Langkah Wajib):</h3>
                 <ol className="list-decimal pl-5 space-y-2 text-blue-900 text-sm">
                   <li>Buka <b>Firebase Console</b> Anda di browser baru.</li>
                   <li>Di menu sebelah kiri, cari menu <b>Build</b>, lalu klik <b>Firestore Database</b>.</li>
                   <li>Di bagian atas halaman, klik tab bertuliskan <b>Rules</b>.</li>
                   <li>Hapus semua teks yang ada di sana, dan <b>gantikan</b> dengan kode (Copy-Paste) di bawah ini:</li>
                 </ol>
                 
                 <div className="bg-gray-900 text-green-400 p-4 rounded-md mt-4 font-mono text-xs overflow-x-auto shadow-inner">
                   rules_version = '2';<br/>
                 ...
                 <p className="text-xs text-red-600 mt-2 font-semibold">Catatan: Menggunakan 'if true;' akan membuka akses uji coba sehingga aplikasi tidak diblokir.</p>

                 <ol className="list-decimal pl-5 space-y-2 text-blue-900 text-sm mt-4" start="5">
                   <li>Terakhir, jangan lupa tekan tombol <b>Publish</b> yang berwarna biru.</li>
                 </ol>
               </div>
             </div>
             ...
             <div className="flex flex-col sm:flex-row gap-3">
               <button 
                 ...
                 className="flex-1 px-4 py-3 bg-emerald-600 text-white font-bold rounded-lg hover:bg-emerald-700 transition-colors shadow-sm text-center"
               >
                 Saya Sudah Publish Rules, Muat Ulang
               </button>
               <button 
                 onClick={() => setIsFallbackMode(true)} 
                 className="flex-1 px-4 py-3 bg-gray-100 text-gray-700 font-bold rounded-lg hover:bg-gray-200 border border-gray-300 transition-colors shadow-sm flex items-center justify-center"
               >
                 <Database className="w-4 h-4 mr-2" /> Coba Offline Saja
               </button>
             </div>
           </div>
        </div>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
         <div className="bg-white p-8 rounded-xl max-w-lg border border-red-200 shadow-lg text-center">
           ...
           <button 
             onClick={() => window.location.reload()} 
             className="w-full px-4 py-3 bg-emerald-600 text-white font-bold rounded-lg hover:bg-emerald-700 transition-colors shadow-sm"
           >
             Muat Ulang Aplikasi
           </button>
         </div>
  ...
  if (!currentUser) {
    ...
    return <LoginView onLogin={(user) => setCurrentUser(user)} accounts={accounts} />;
  }
    ...
    <div className="flex min-h-screen bg-gray-50 font-sans print:bg-white">
      ...
      <div className="flex-1 flex flex-col min-h-screen">
        ...
        <main className="flex-1 p-4 sm:p-6 lg:p-8 overflow-y-auto print:p-0 print:m-0 print:overflow-visible">
          ...
          {currentView === 'account' && <AccountSettingsView currentUser={currentUser} accounts={accounts} setCurrentUser={setCurrentUser} />}
        </main>
      </div>
    </div>
  );
